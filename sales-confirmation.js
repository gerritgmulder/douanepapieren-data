...
(function (global) {
  ...
  /* Ruimer, voor proforma's van andere fabrieken: een code zonder spaties,
     met minstens één cijfer, hooguit twintig tekens. "GL8948", "YHL-2608",
     "100092", "WS-PC08T". Geen woorden dus, en geen bedragen met een punt en
     twee decimalen erachter. */
  var CODE_RUIM = /^([A-Z0-9][A-Z0-9\-\/\.]{2,19})\s*:?\s*$/i;
  function isCode(s) {
    s = schoon(s);
    if (!s || !CODE_RUIM.test(s) || !/\d/.test(s)) return false;
    if (/^\d+[.,]\d{1,2}$/.test(s)) return false;         // een bedrag
    if (/^\d{4}[-\/]\d{1,2}[-\/]\d{1,2}$/.test(s)) return false;   // een datum
    return true;
  }
  ...
  function leesArtikelcel(waarde) {
    ...
    for (var i = 0; i < regels.length; i++) {
      var m = regels[i].match(ARTIKEL) || (isCode(regels[i]) ? regels[i].match(CODE_RUIM) : null);
      if (m) {
        laatste = { code: m[1].toUpperCase().replace(/:$/, ""), maat: "", perSet: 1 };
        codes.push(laatste);
        continue;
      }
  ...
  /* De kopregel mag er bij elke fabriek anders uitzien. Chantal (video,
     16 sep 2026): "als ik van al onze honderd fabrieken eerst alles naar het
     systeem moet sturen om te laten lezen, dan gaat dit wel echt heel veel
     tijd kosten. Kan je gewoon zorgen dat die pro forma invoices gewoon
     gelezen kunnen worden." Daarom niet één kopje per kolom maar een rijtje
     woorden dat de fabrieken ervoor gebruiken, en de kop mag tot rij 40
     staan (boven de tabel staat vaak een half adresboek). Een kop die over
     twee rijen is verdeeld ("order" boven "quantity") wordt gevangen doordat
     regeleinden in een cel als spatie tellen. */
  function kolommen(rijen) {
    for (var r = 0; r < Math.min(rijen.length, 80); r++) {
      var k = { rij: r };
      for (var c = 0; c < (rijen[r] || []).length; c++) {
        var v = schoon(cel(rijen, r, c)).toLowerCase();
        if (!v) continue;
        if (/^item\s*(no|code|#|number)?\.?:?$|^(art(ikel|icle)?|product|model|style)\s*(no|code|number|#)?\.?:?$|^(item|art)\s*nr\.?:?$|^code:?$|^model:?$/.test(v)) { if (k.artikel === undefined) k.artikel = c; }
        else if (/^colou?r$|^kleur$|^finish$/.test(v)) k.kleur = c;
        else if (/^(order\s*)?(quan?[it]?ity|qty\.?|q'ty|aantal|pcs)(\s*[\/\(]\s*(pcs|sets?|units?)\)?)?$/.test(v)) { if (k.aantal === undefined) k.aantal = c; }
        else if (/^description|^items?$|^item\s*name|^product\s*name|^omschrijving|^goods/.test(v)) { if (k.omschrijving === undefined) k.omschrijving = c; }
      }
      if (k.artikel !== undefined && k.aantal !== undefined) return k;
    }
    return null;
  }
  /* Hetzelfde document onder een andere naam: een proforma invoice zegt ook
     welke artikelen in welke aantallen besteld zijn. */
  function isProforma(rijen) {
    var kop = (rijen || []).slice(0, 40).map(function (r) { return (r || []).join(" "); }).join(" ").toLowerCase();
    return /pro\s*-?\s*forma|\bp\.?\s*\/?\s*i\.?\s*no|\bpi\s*no|supplier\s*invoice|order\s*confirmation|purchase\s*order/.test(kop);
  }
  ...
  function isSalesConfirmation(rijen) {
    ...
    if (!/sales\s*confirmation/.test(kop) && !/s\/c\s*no/.test(kop) && !isProforma(rijen)) return false;
  ...
  function lees(rijen) {
    ...
    function naastLabel(patroon) {
      for (var r = 0; r < Math.min(rijen.length, 40); r++) {
        for (var c = 0; c < (rijen[r] || []).length; c++) {
    ...
    uit.referentie = naastLabel(/^s\/c\s*no\.?:?$/i)
      || naastLabel(/^(p\.?\/?i\.?|pi|proforma(\s*invoice)?|supplier\s*invoice|invoice|order|p\.?o\.?)\s*(no|nr|number|#)?\.?:?$/i)
      || inLabel(/^(?:p\.?\/?i\.?|pi|proforma(?:\s*invoice)?|invoice|s\/c|order)\s*(?:no|nr|number|#)?\.?\s*[:：]\s*(\S.{1,40})$/i)
      || inLabel(/^no\.?\s*[:：]?\s*((?:pi|p\/i|s\/c|sc)[-\/\s]?\S{2,40})$/i);
    /* Eerst "Order date: 2026-8-21" in één cel, dan een los label met de
       waarde ernaast. Een waarde die zelf een label is ("Date:" naast "Date:"
       in de handtekeningregel) telt niet. */
    var datum = inLabel(/^(?:order\s*|invoice\s*)?date\s*[:：]\s*(\S.{1,30})$/i) || naastLabel(/^(order\s*|invoice\s*)?date:?$/i);
    if (datum && /:$/.test(datum)) datum = null;
    /* "PI No.: LS6J821" in één cel: het label en de waarde samen. */
    function inLabel(patroon) {
      for (var r = 0; r < Math.min(rijen.length, 40); r++)
        for (var c = 0; c < (rijen[r] || []).length; c++) {
          var m = schoon(cel(rijen, r, c)).match(patroon);
    ...
    for (var i = k.rij + 1; i < rijen.length; i++) {
      ...
      var ruwAantal = schoon(cel(rijen, i, k.aantal)).replace(/\s*(pcs?|sets?|stuks?|units?)\.?$/i, "");
